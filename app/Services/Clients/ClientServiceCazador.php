...
/**
 * Servicio de clientes solo para la aplicación Cazador (API).
 * Crea/actualiza clientes asignados al cazador autenticado (assigned_advisor_id).
 */
class ClientServiceCazador
{
    public function createClient(array $formData, ?int $createdById = null): Client
    {
        try {
            ...
            Log::info("Cliente cazador creado ID: {$client->id} (API)");
            return $client;
        } catch (ValidationException $e) {
            throw $e;
        } catch (\Exception $e) {
            Log::error('Error al crear cliente cazador (API): ' . $e->getMessage());
    ...
    public function updateClient(int $id, array $formData): bool
    {
        try {
            ...
            if ($updated) {
                Log::info("Cliente cazador actualizado ID: {$id} (API)");
            }
            return (bool) $updated;
        } catch (ValidationException $e) {
            throw $e;
        } catch (\Exception $e) {
            Log::error("Error al actualizar cliente cazador ID {$id} (API): " . $e->getMessage());
    ...
    protected function validateClientData(array $data, ?int $clientId = null, ?string $createMode = null): void
    {
        ...
        if ($validator->fails()) {
            Log::warning('Validación fallida cliente cazador (API)', ['errors' => $validator->errors()->toArray()]);
